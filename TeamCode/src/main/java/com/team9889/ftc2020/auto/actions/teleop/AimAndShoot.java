...
public class AimAndShoot extends Action {
    ...
    Point posOfTarget = new Point(-11, 65);
    ...
    @Override
    public void start() {
        paths.set(0, new Path(Robot.getInstance().getMecanumDrive().getCurrentPose(),
                paths.get(0).getTolerancePose(), paths.get(0).getRadius(), paths.get(0).getMaxVelocity()));
        paths.set(1, new Path(new Pose2d(Robot.getInstance().getMecanumDrive().getCurrentPose().getX(),
                Robot.getInstance().getMecanumDrive().getCurrentPose().getY(),
                CalculatePose( new Point(Robot.getInstance().getMecanumDrive().getCurrentPose().getX(),
                        Robot.getInstance().getMecanumDrive().getCurrentPose().getY()))),
                paths.get(0).getTolerancePose(), paths.get(0).getRadius(), paths.get(0).getMaxVelocity()));
    }
    ...
    double CalculatePose (Point robotPos) {
        ...
        double angle = Math.asin(offset.y / hypotenuse);
